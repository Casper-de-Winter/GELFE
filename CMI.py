# -*- coding: utf-8 -*-
"""
Created on Wed May 15 15:20:58 2019

@author: casper
"""

from entropy_estimators import midd,cmidd
import numpy as np
from copy import deepcopy
from sklearn.preprocessing import KBinsDiscretizer

def cife(X2, y, XNumind, fixed = 0, **kwargs):
    """
    ...
    X = deepcopy(X2)
    for num in XNumind:
        kbd = KBinsDiscretizer(encode = 'ordinal', strategy = 'uniform', n_bins = 8)
        X[:,num] = kbd.fit_transform(X[:,num].reshape(-1,1))[:,0]
    ...
    for i in range(n_features):
        ...
        if i in XNumind:
            f2 = X2[:,i]
            t1[i] = (3*t1[i] + np.abs(np.corrcoef(f2, y)[0][1])) / 4
        ...
        if fixed == 0:
            ...
            for i in range(n_features):
                if i not in F:
                    ...
                    t3[i] += cmidd(f_select, f, y)
                    if first:
                        j_cmi = t1[i] - beta*t2[i] + gamma*t3[i]
                        idx = i
                        first = False
            F.append(idx)
            J_CMI.append(j_cmi)
